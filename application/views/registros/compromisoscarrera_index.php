	      		...
	      		<div class="widget">
					...
					<div class="widget-content">
						<div id="formsearch" class="">
							...
							$ruta = 'registros/compromisoscarrera/buscar';
							...
							<table class="table table-striped table-bordered">
				                ...
				                <tbody>
				                	...
				                	if(!isset($offset))
				                		$offset=0;
				                	if(isset($data_ccar)){
				                		foreach ($data_ccar as $item => $compcarrera) {
				               		?>
										<tr>
											<td class="texto-centrado"><?= str_pad(($item+1)+$offset, 5, '0', STR_PAD_LEFT); ?></td>
											<td class=""><?= $compcarrera->ccar_descripcion; ?></td>
											<td class="texto-centrado td-actions">
						                    	<a title="Ver" class="btn btn-small btn-info btn_consulta" href="<?= base_url('registros/compromisoscarrera/ver/'.str_encrypt($compcarrera->ccar_id, KEY_ENCRYPT)); ?>">
						                    		<i class="btn-icon-only <?= ICON_VIEW; ?>"> </i>
						                    	</a>
						                    	<a title="Editar" class="btn btn-small btn-invert btn_editar" href="<?= base_url('registros/compromisoscarrera/editar/'.str_encrypt($compcarrera->ccar_id, KEY_ENCRYPT)); ?>">
						                    		<i class="btn-icon-only <?= ICON_EDIT; ?>"> </i>
						                    	</a>
						                    	<a title="Eliminar" class="btn btn-small btn-danger tr_delete" href="javascript:;" data-url="<?= base_url('registros/compromisoscarrera/eliminar/'.str_encrypt($compcarrera->ccar_id, KEY_ENCRYPT)); ?>">
