...
class Carrera extends CI_Controller {
    ...
    public function ver($carr_id_enc = ''){
        ...
        $data['data_modu']      = $this->modulosxcarrera_model->getModulosxcarreraByCARRID($carr_id);
    ...
    public function guardar($carr_id_enc = ''){
        ...
        else
        {
            ...
            if($data_response){
                ...
                $carr_id = $data_response;
                redirect('registros/carrera/ver/'.str_encrypt($carr_id, KEY_ENCRYPT));
            }else{
                $this->session->set_flashdata('mensaje_tipo', EXIT_ERROR);
                $this->session->set_flashdata('mensaje', RMESSAGE_ERROR);
                $this->layout->view('registros/carrera_form', $data);
